...
const transporter = nodemailer.createTransport({
  ...
  secure: true,
  auth: {
    user: process.env.MAIL_GOOGLE_USER_EMAIL,
    pass: process.env.MAIL_GOOGLE_USER_APP_PASSWORD,
  },
});
...
export const sendVerificationEmail = async (email: string, token: string) => {
  ...
  // send email
  const mailOptions = {
    from: process.env.MAIL_GOOGLE_USER_EMAIL,
    to: email,
    ...
    html: htmlOne({
      ...
      email,
      label: "Click to confirm your registration",
    }),
  };
